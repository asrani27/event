...
<div class="p-6">
    ...
    <div class="mb-8">
        ...
        <h2 class="text-2xl font-bold text-gray-800 mb-2">Edit Event</h2>
        <p class="text-gray-600">Perbarui informasi event yang ada</p>
    </div>
        ...
        <form action="{{ route('admin.events.update', $event) }}" method="POST" class="space-y-6">
            ...
            <div>
                ...
                <textarea 
                    ...
                    required
                    class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-purple-600 focus:border-transparent transition duration-200"
                    placeholder="Deskripsikan event Anda secara detail">{{ old('description', $event->description) }}</textarea>
                @error('description')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div>
                <label for="current_participants" class="block text-sm font-medium text-gray-700 mb-2">
                    Peserta Saat Ini <span class="text-red-500">*</span>
                </label>
                <input type="number" 
                    id="current_participants" 
                    name="current_participants" 
                    value="{{ old('current_participants', $event->current_participants) }}"
                    min="0"
                    max="{{ old('max_participants', $event->max_participants) }}"
                    required
                    class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-purple-600 focus:border-transparent transition duration-200"
